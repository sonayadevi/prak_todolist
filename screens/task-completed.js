...
const TaskCompletedScreen = () => {
    ...
    newList[index].isCompleted = !newList[index].isCompleted; setAllList(newList);
    try {
    AsyncStorage.setItem("@task-list", JSON.stringify(newList));
    } catch (e) {
    console.log("Error update status task: in task-completed.js"); console.error(e.message);
    } finally {
    ...
    console.log("Error get task: in task-completed.js"); console.error(e);
    } finally {
    setIsLoading(false);
    }
    };
...
<ScrollView>
{allList.map((item, index) => { if (item.isCompleted) {
return (
